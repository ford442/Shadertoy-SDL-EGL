...
return[P[0],P[1],P[2],(1.0-((p[0]+p[1]+p[2])/3))];}).setTactic("speed").setPipeline(true).setOutput(o);
const r=g.createKernel(function(f){const p=f[this.thread.y][this.thread.x];
this.color(p[0],p[1],p[2],p[3]*(Math.trunc(p[3]/0.25)));}).setTactic("speed").setGraphical(true).setOutput(o);
