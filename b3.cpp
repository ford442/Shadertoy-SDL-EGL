...
const t=g.createKernel(function(v,alf){const P=v[this.thread.y][this.thread.x];
return[P[0],P[1],P[2],0.7];}).setTactic("speed").setPipeline(true).setOutput(o);
const r=g.createKernel(function(f){const p=f[this.thread.y][this.thread.x];
this.color(p[0],p[1],p[2],0.7);}).setTactic("speed").setGraphical(true).setOutput(o);
