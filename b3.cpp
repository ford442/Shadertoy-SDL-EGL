...
var t=g.createKernel(function(v){const P=v[this.thread.y][this.thread.x+500];const aveg=1.0-((((P[0]+P[1]+P[2])/3)-0.75)*4.0);return[P[0],P[1],P[2],(aveg)];}).setTactic("precision").setPipeline(true).setDynamicOutput(true).setOutput(o);
var r=g.createKernel(function(f){const p=f[this.thread.y][this.thread.x+500];this.color(p[0],p[1],p[2],p[3]);}).setTactic("precision").setGraphical(true).setDynamicOutput(true).setOutput(o);
