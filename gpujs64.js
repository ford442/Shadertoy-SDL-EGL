...
const row = new Float64Array(this.output.x);
...
break;
default:
retArr.push(`new Float64Array([${elements.join(', ')}])`);
}
return retArr;
}
...
const readBackValue = kernel.precision === 'single' ? targetName : `new Float64Array(${targetName}.buffer)`;
if (kernel.output[2]) {
...
result = new Float64Array(w * h * 4 * 4);
context.readPixels(0, 0, w, h * 4, context.RGBA, context.FLOAT, result);
} else {
const bytes = new Uint8Array(texSize[0] * texSize[1] * 4);
context.readPixels(0, 0, texSize[0], texSize[1], context.RGBA, context.UNSIGNED_BYTE, bytes);
result = new Float64Array(bytes.buffer);
}
...
this.renderRawOutput = this.readPackedPixelsToUint8Array;
this.transferValues = this.readPackedPixelsToFloat64Array;
if (this.pipeline) {
...
this.renderRawOutput = this.readFloatPixelsToFloat64Array;
this.transferValues = this.readFloatPixelsToFloat64Array;
if (this.pipeline) {
this.renderOutput = this.renderTexture;
if (this.subKernels !== null) {
this.renderKernels = this.renderKernelsToTextures;
...
readPackedPixelsToFloat64Array() {
return new Float64Array(this.readPackedPixelsToUint8Array().buffer);
}

readFloatPixelsToFloat64Array() {
...
const w = texSize[0];
const h = texSize[1];
const result = new Float64Array(w * h * 4);
...
const result = new Float64Array(size[0] * size[1] * 4);
...
renderValues() {
if (this._deleted) return null;
return new Float64Array(this.renderRawOutput().buffer);
}
toArray() {
...
this.uploadValue = new Float64Array(this.uploadArrayLength);
this.kernel.setUniform3iv(this.dimensionsId, this.dimensions);
this.kernel.setUniform2iv(this.sizeId, this.textureSize);
super.updateValue(value);
}
...
getStringValueHandler() {
return utils.linesToString([
`const uploadValue_${this.name} = new Float64Array(${this.uploadArrayLength})`,
...
const vertices = new Float64Array([-1, -1,
1, -1, -1, 1,
1, 1
]);
const texCoords = new Float64Array([
0, 0,
1, 0,
0, 1,
1, 1
...
initContext() {
const settings={colorType:'float32',preferLowPowerToHighPerformance:false,precision:'highp',logarithmicDepthBuffer:true,colorSpace:'display-p3',alpha:true,depth:true,stencil:true,imageSmoothingEnabled:true,preserveDrawingBuffer:true,premultipliedAlpha:true,desynchronized:false,lowLatency:true,powerPreference:'high-performance',antialias:true,willReadFrequently:true,majorVersion:2,minorVersion:0};
...
const [w, h, d] = this.size;
if (d) {
return utils.erectMemoryOptimized3DFloat(this.value.subarray ? this.value : new Float64Array(this.value), w, h, d);
} else if (h) {
return utils.erectMemoryOptimized2DFloat(this.value.subarray ? this.value : new Float64Array(this.value), w, h);
} else {
return this.value;
}
}
...
const xResults = new Float64Array(width);
let i = 0;
for (let x = 0; x < width; x++) {
xResults[x] = array[i];
i += 4;
