...
auto current_time=std::chrono::duration<double>(std::chrono::system_clock::now());
double NowTime=current_time.count();

SDL_Log("SDL_Time: %u",SDL_GetTicks());
sleep(1);
cout << "Elapsed time in nanoseconds A: "<< NowTime << " ns." << endl;
sleep(1);
auto current_timeb=std::chrono::duration<double>(std::chrono::system_clock::now());
double NowTimeB=current_timeb.count();
