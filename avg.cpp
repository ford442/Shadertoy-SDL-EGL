...
}).setTactic("speed").setDynamicOutput(true);
...
}).setTactic("balanced").setPipeline(true).setDynamicOutput(true);
  
let r=g.createKernel(function(f){
const p=f[this.thread.y][this.thread.x];
this.color(p[0],p[1],p[2],p[3]);
}).setTactic("balanced").setGraphical(true).setDynamicOutput(true);
