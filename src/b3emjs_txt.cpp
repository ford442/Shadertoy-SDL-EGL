...
var cvv=document.getElementById("mvc");
...
var locb=$Bu+1;
if(locb>64){locb=1;}
eval("if($F=="+i+"){var $r"+i+"=t($"+i+");r($r"+i+");var $$"+$Bu+"=t(cvv);$"+$Bu+".set($$"+$Bu+");$F="+loca+";$Bu="+locb+";}");
}
pointb=66*la;
if($F%2==0){
...
vv.currentTime-=0.013333;
}else{
f=false;
if(vv.currentTime<(stp+2.0)){
vv.currentTime+=0.013333;
}else{
f=true;
}}}
