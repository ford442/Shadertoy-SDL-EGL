...
var s$=600; // parseInt(inh,10);
var w$=600; // parseInt(inh,10);
var h$=600; // parseInt(inh,10);
...
let g=new GPUX({mode:"gpu",canvas:bcanvas,webGl:gljs});
...
var d=S();if(d){d()};d=S();function S(){
    ...
    blank$=Math.max((((w$-s$)*1.0)/8.0),0.0);
    nblank$=Math.max((((s$-w$)*1.0)/8.0),0.0);
    s$=600; // parseInt(window.innerHeight,10);
    la=Math.ceil(h$*w$*8);
    pointa=Math.ceil(77*la);
