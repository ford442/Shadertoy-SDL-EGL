...
let S$=parseInt(document.getElementById("pmhig").innerHTML,10);
...
const contx=bcanvas.getContext("webgl2",{colorType:'float64',precision:'highp',colorSpace:'display-p3',alpha:true,depth:true,stencil:true,preserveDrawingBuffer:true,premultipliedAlpha:true,desynchronized:false,lowLatency:false,powerPreference:'high-performance',antialias:true,willReadFrequently:false});
...
}).setTactic("precision").setPipeline(true).setArgumentTypes(["HTMLVideo"]).setDynamicOutput(true).setOutput([S$,S$]).setStrictIntegers(false).setFixIntegerDivisionAccuracy(false);
...
}).setTactic("precision").setGraphical(true).setArgumentTypes(['HTMLVideo']).setDynamicOutput(true).setOutput([S$,S$]).setStrictIntegers(false).setFixIntegerDivisionAccuracy(false);
