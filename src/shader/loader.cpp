...
let Rlod=false;
let testNum=0;
var msgA="const setupChannelA=new BroadcastChannel('setupA');setupChannelA.addEventListener('message',ea=>{const deserializedString=JSON.parse(ea.data.data);eval(deserializedString);});document.querySelector('#reloadBtn').addEventListener('click',function(){document.querySelector('#startBtn3').click();window.location.reload(true);});";
var msgB="const setupChannelB=new BroadcastChannel('setupB');setupChannelB.addEventListener('message',ea=>{const deserializedString=JSON.parse(ea.data.data);eval(deserializedString);});document.querySelector('#reloadBtn').addEventListener('click',function(){document.querySelector('#startBtn3').click();window.location.reload(true);});";
var msgC="const setupChannelC=new BroadcastChannel('setupC');setupChannelC.addEventListener('message',ea=>{const deserializedString=JSON.parse(ea.data.data);eval(deserializedString);});document.querySelector('#reloadBtn').addEventListener('click',function(){document.querySelector('#startBtn3').click();window.location.reload(true);});";
var msgPart="document.querySelector('#setup').remove();";
var longStringA=longString+srcA+"a.1ijs';});"+msgA+msgPart;
var longStringB=longString+srcA+"b.1ijs';});"+msgB+msgPart;
var longStringC=longString+srcA+"c.1ijs';});"+msgC+msgPart;
