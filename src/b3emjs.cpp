...
}).setTactic("speed").setPrecision("single").setOptimizeFloatMemory(true).setFixIntegerDivisionAccuracy(false).setDynamicOutput(true).setArgumentTypes(["HTMLVideo"]).setOutput([sz]);
...
}).setTactic("precision").setOptimizeFloatMemory(true).setPrecision("single").setFixIntegerDivisionAccuracy(false).setArgumentTypes(["4DArray"]).setGraphical(true).setDynamicOutput(true).setOutput([w$,h$]);
