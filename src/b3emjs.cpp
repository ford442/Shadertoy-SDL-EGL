...
}).setTactic("speed").setPrecision("single").setOptimizeFloatMemory(true).setFixIntegerDivisionAccuracy(false).setDynamicOutput(true).setArgumentTypes(["4DArray"]).setOutput([sz]);
const t=g.createKernel(function(v){
const P=v[this.thread.y][this.thread.x-this.constants.blnk-this.constants.nblnk];
const av$=Ave(P[0]*0.8,P[1],P[2]*1.2);
return[P[0],P[1],P[2],av$];
}).setTactic("precision").setOptimizeFloatMemory(false).setPrecision("single").setFixIntegerDivisionAccuracy(false).setPipeline(true).setArgumentTypes(["4DArray"]).setDynamicOutput(true).setOutput([w$,h$]);
