...
}).setTactic("speed").setPrecision("single").setOptimizeFloatMemory(false).setFixIntegerDivisionAccuracy(false).setDynamicOutput(true).setArgumentTypes(["4DArray"]).setOutput([sz]);
...
}).setTactic("precision").setOptimizeFloatMemory(true).setPrecision("single").setFixIntegerDivisionAccuracy(false).setGraphical(true).setArgumentTypes(["4DArray"]).setDynamicOutput(true).setOutput([w$,h$]);
