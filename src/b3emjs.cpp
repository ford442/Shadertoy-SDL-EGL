...
}).setTactic("speed").setPrecision("single").setOptimizeFloatMemory(false).setFixIntegerDivisionAccuracy(false).setDynamicOutput(true).setArgumentTypes(["Number"]).setOutput([sz]);
const t=g.createKernel(function(v){
const P=v[this.thread.y][this.thread.x-this.constants.blnk-this.constants.nblnk];
const av$=Ave(P[0]*0.8,P[1],P[2]*1.2);
return[P[0],P[1],P[2],av$];
}).setTactic("precision").setOptimizeFloatMemory(false).setPrecision("single").setFixIntegerDivisionAccuracy(false).setPipeline(true).setArgumentTypes(["HTMLVideo"]).setDynamicOutput(true).setOutput([w$,h$]);
...
}).setTactic("precision").setOptimizeFloatMemory(false).setPrecision("single").setFixIntegerDivisionAccuracy(false).setGraphical(true).setArgumentTypes(["Array3D(4)"]).setDynamicOutput(true).setOutput([w$,h$]);
