...
@group(0)@binding(7)var <storage,read_write> mappedBuffer: array<f32,64>;
...
WGPU_Range_PointerB=wgpu_buffer_get_mapped_range(WGPU_Buffers.at(2,0,2),0,256);
...
bufferDescriptorM={OutputBufferBytesU,WGPU_BUFFER_USAGE_STORAGE|WGPU_BUFFER_USAGE_MAP_READ|WGPU_BUFFER_USAGE_COPY_DST,false};
...
Compute_Bindgroup_Layout_Entries[6].binding=7;
Compute_Bindgroup_Layout_Entries[6].visibility=WGPU_SHADER_STAGE_COMPUTE;
Compute_Bindgroup_Layout_Entries[6].type=WGPU_BIND_GROUP_LAYOUT_TYPE_BUFFER;
Compute_Bindgroup_Layout_Entries[6].layout.buffer=WGPU_BufferBindingLayout.at(0,0,5);
...
Compute_Bindgroup_Entries[6].binding=7;
Compute_Bindgroup_Entries[6].resource=WGPU_Buffers.at(2,0,2);
Compute_Bindgroup_Entries[6].bufferBindOffset=0;
Compute_Bindgroup_Entries[6].bufferBindSize=OutputBufferBytes;
