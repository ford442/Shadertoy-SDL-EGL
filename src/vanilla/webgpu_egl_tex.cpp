...
"for(var m:u32=thread_id.x;m<8u;m=m+1u){\n"
"var loopY:f32=loopX*f32(thread_id.x);\n"
"for(var x:f32=0.0f;x<=loopX;x=x+1.0f){\n"
  "var INxPos:u32=u32(round(x+loopY*sizeRatio));\n"
  "var OUTxPos:u32=u32(x+loopY);\n"
  "for(var y:f32=0.0f;y<=loopX;y=y+1.0f){\n"
    "var INyPos:u32=u32(round(y+loopY*sizeRatio));\n"
    "var OUTyPos:u32=u32(y+loopY);\n"
    "var INtexCoord:vec2<u32>=vec2<u32>(INxPos,INyPos);\n"
    "var color:vec4<f32>=textureLoad(textureIN,INtexCoord,0);\n"
    "textureStore(videoOUT,vec2<u32>(OUTxPos,OUTyPos),color);\n"
  "}\n"
"}\n"
"}\n"
"}";
