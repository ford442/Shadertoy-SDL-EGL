...
wgpu_render_pass_encoder_set_vertex_buffer(wrpe.at(1,1),0,wb.at(8,8),0,sizeof(UVvertices));
...
wbd.at(6,6)=bufferDescriptor_vertex;
  bufferDescriptor_vertex_UV.size=sizeof(UVvertices);
bufferDescriptor_vertex_UV.usage=WGPU_BUFFER_USAGE_VERTEX|WGPU_BUFFER_USAGE_COPY_DST;
bufferDescriptor_vertex_UV.mappedAtCreation=EM_FALSE;
  wbd.at(8,8)=bufferDescriptor_vertex_UV;
vertAtt.offset=0;
...
wva.at(0,0)=&vertAtt;
wva.at(1,1)=&vertAtt2;
vertBufLayouts[0].numAttributes=1;
vertBufLayouts[0].attributes=&vertAtt;
vertBufLayouts[0].arrayStride=sizeof(Vertex);
vertBufLayouts[0].stepMode=WGPU_VERTEX_STEP_MODE_VERTEX;
vertBufLayouts[1].numAttributes=1;
vertBufLayouts[1].attributes=&vertAtt2;
vertBufLayouts[1].arrayStride=sizeof(VertexUV);
vertBufLayouts[1].stepMode=WGPU_VERTEX_STEP_MODE_VERTEX;
wvbl.at(0,0)=vertBufLayouts;
// wvbl.at(1,1)=vertBufLayoutUV;
vertex_Buffer=wgpu_device_create_buffer(wd.at(0,0),&wbd.at(6,6));
wb.at(6,6)=vertex_Buffer;
vertex_Buffer_UV=wgpu_device_create_buffer(wd.at(0,0),&wbd.at(8,8));
wb.at(8,8)=vertex_Buffer;
...
wgpu_queue_write_buffer(wq.at(0,0),wb.at(8,8),0,UVvertices,sizeof(UVvertices));
