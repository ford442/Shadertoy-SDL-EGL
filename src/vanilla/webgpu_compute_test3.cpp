...
"var texTest:vec4<f32>=textureSample(textureIN,resizeSampler,vec2<u32>(x,y));\n"
"var colorTest:vec4<f32>=textureLoad(textureIN,vec2<u32>(x,y),0);\n"
"textureStore(textureOUT,vec2<f32>(x,y),clr);\n"
"outputBuffer[3]=texTest.x;\n"
"}"
"}"
"}";
...
textureDescriptorIn.width=720; // sze.at(1,1);
textureDescriptorIn.height=720; // sze.at(1,1); // default = 1;
