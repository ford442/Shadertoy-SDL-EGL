...
"@vertex\n"
"fn main(@location(0) in vec4<f32> position : vec4<f32>) -> @builtin(position) vec4<f32> {"
"return position;}\n";
...
bufferBindingLayoutV.type=WGPU_BIND_GROUP_LAYOUT_TYPE_BUFFER;
...
bindgroup_layout_entries[7].binding=7;
bindgroup_layout_entries[7].visibility=WGPU_SHADER_STAGE_VERTEX;
bindgroup_layout_entries[7].type=WGPU_BIND_GROUP_LAYOUT_TYPE_BUFFER;
bindgroup_layout_entries[7].layout.buffer=wbbl.at(1,1);
wbgle.at(0,0)=bindgroup_layout_entries;
bindgroup_layout=wgpu_device_create_bind_group_layout(wd.at(0,0),wbgle.at(0,0),8);
...
bindgroup_entries[7]={};
bindgroup_entries[7].binding=7;
bindgroup_entries[7].resource=wb.at(3,3);
...
bindgroup=wgpu_device_create_bind_group(wd.at(0,0),wbgl.at(0,0),wbge.at(0,0),8);
