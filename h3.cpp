...
attr.imageSmoothingEnabled=EM_FALSE;
...
const glslAlphe=`float Alphe(float a,float b,float c,float d,float e,float f,float g) {return ((g + (((c - d) * 0.75) + d) + (((1.0 - (b / 2.0)) * 0.75) + (b/2.0)) + (((1.0 - (c)) * 0.75)) + ((0.75 - (0.75 * (e - f) / (c - f)))) + ( f ) / 6.0) ;}`;
...
var avrg=(1.0-Ave(pd[0],pd[1],pd[2])-0.8)*(Ave(pd[0],pd[1],pd[2])*5.0);
