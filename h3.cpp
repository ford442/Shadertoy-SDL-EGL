...
const glslAlphe=`float Alphe(float a,float b,float c,float d,float e,float f,float g) {return ((((1.0*floor(0.5/((a-b)*(c-d))+b))*(((a-b)*g)+b))+(((1.0*(ceil(0.5/f)))*(1.0-(((a-e)*g)+e)))+((1.0*(ceil(0.5/f)))*(((a-d)*(c-d))+d)))+((((c-d)*(1.0-a))+(((1.0-b)*g)+b))/2.0)+(1.0-(f-g))+(c-d))/5.0);}`;
...
var Min=(($amin*($amax-$aavg)/2.0)+(($amax-$aavg)/2.0)+$aavg);
