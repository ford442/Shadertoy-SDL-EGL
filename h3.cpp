...
const glslAlphe=`float Alphe(float a,float b,float c,float d,float e,float f,float g) {return 1.0-((((1.0*floor(0.5/((c-((c-d)*0.25))*(c-((c-d)*0.25)))+b))*(((c-b)*g)+b))+(((1.0*(ceil(0.5/f)))*(1.0-(((a-e)*g)+e)))+((1.0*(ceil(0.5/f)))*(((a-d)*(a-d))+d)))+((((a)*(1.0-a))+(((1.0-b)*g)+b))/2.0)+(1.0-(f-g))+(c-(c*(1.0-c))))/5.0);}`;
...
var alph=Alphe($amax,$amin,$fmax,$fmin,$aavg,$aavg,p[3]);
var Min=(($fmin*($amax-$favg)/2.0)+(($amax-$aavg)/2.0)+$aavg);
