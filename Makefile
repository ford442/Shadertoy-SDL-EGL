...
#LINK_SIMD_FLAGS += -msimd128
LINK_SIMD_FLAGS += -msimd128 -mavx
# -mcpu=bleeding-edge
COMMON_FLAGS += -mdouble=64 -fopenmp -sSUPPORT_LONGJMP=emscripten -sDEFAULT_TO_CXX=0 -pthread -pipe -mextended-const -mbulk-memory -matomics \
...
LINK_FLAGS += $(LDFLAGS) -DDOUBLE -sWASM_BIGINT=0 -sLEGALIZE_JS_FFI=0 -sMALLOC=emmalloc -sOFFSCREENCANVAS_SUPPORT=1 -sTEXTDECODER=0 -sALLOW_TABLE_GROWTH=1 -sEMULATE_FUNCTION_POINTER_CASTS=0 -sSUPPORT_BIG_ENDIAN=1 \
