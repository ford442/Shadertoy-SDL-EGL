...
COMMON_FLAGS += -sSUPPORT_LONGJMP=wasm -sDEFAULT_TO_CXX=0 -pthread -pipe -mextended-const -mbulk-memory -matomics -sUSE_ICU=1 -sWASM_WORKERS=1 -sSHARED_MEMORY=1 \
...
GL_FLAGS += -sFULL_ES2=1 -sFULL_ES3=1 -sUSE_GLFW=0 -Wl,-lGL,-lGLU,-lEGL,-lGLESv2 -sUSE_REGAL=0 \
-sUSE_WEBGL2=1 -sMIN_WEBGL_VERSION=2 -sMAX_WEBGL_VERSION=2 -sGL_TRACK_ERRORS=0

LINK_FLAGS += $(LDFLAGS) -sTRUSTED_TYPES=1 -sALLOW_UNIMPLEMENTED_SYSCALLS=0 -sIGNORE_MISSING_MAIN=0 -sABORT_ON_WASM_EXCEPTIONS=0 -sDEMANGLE_SUPPORT=0 -sASSERTIONS=2 \
	 -sTEXTDECODER=1 --use-preload-plugins --closure 0 --closureFriendly \
	 -march=haswell -sWASM=1 -sWASM_PAGE_SIZE=16000 -sTOTAL_STACK=128000 -sENVIRONMENT='web,worker' -sSTRICT_JS=1 \
	 -sGLOBAL_BASE=352321536 -sSUPPORT_ERRNO=0 -DNDEBUG=1 -polly -polly-position=before-vectorizer \
	 -sALLOW_MEMORY_GROWTH=0 -sMAXIMUM_MEMORY=3221225472 --output_eol linux -mllvm -mtune=haswell -wasm-enable-eh \
...
b3_vanilla_webgpu:
	 ...
	 em++ -fno-except -DLIB_WEBGPU -DLIB_WEBGPU_CPP20 main_webgpu.o libonnxruntime_webassembly.a -O3 -o w3001.js \
	 -std=c++17 $(BOOST_FLAGS) $(LINK_SIMD_FLAGS) -sFORCE_FILESYSTEM=1 \
	 -fwhole-program-vtables -polly -sALLOW_MEMORY_GROWTH=0 -sDISABLE_EXCEPTION_THROWING=0 \
	 -sINITIAL_MEMORY=768mb -lmath.js -lhtml5.js -lint53.js \
	 -sSUPPORT_ERRNO=0 \
	 -sUSE_WEBGL2=1 -sMIN_WEBGL_VERSION=2 -sMAX_WEBGL_VERSION=2 \
	 -sASYNCIFY=1 -sASYNCIFY_IMPORTS=['startWebGPU','runWebGPU','wgpu_buffer_map_sync','navigator_gpu_request_adapter_sync','wgpu_adapter_request_device_sync'] \
...
b3_audio_mk:
	...
	emcc $(STDS) -o a3020.js main.o audio.o $(COMMON_FLAGS) -O2 $(LINK_FLAGS) $(LINK_SIMD_FLAGS) $(BOOST_FLAGS) \
	-sUSE_SDL=2 -sUSE_SDL_IMAGE=0 -sUSE_SDL_TTF=0 -sUSE_SDL_NET=0 \
