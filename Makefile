b3hd:
	 ...
	 em++ src/b3sdl.cpp -shared -o sdl.so -sSIDE_MODULE=2 -sWASM_WORKERS \
	 ...
	 -mmultivalue -mextended-const -fno-signed-zeros -freciprocal-math -ffp-contract=off -Xclang -menable-no-nans -Xclang -menable-no-infs \
	 -ffp-exception-behavior=maytrap -ffast-math -ffp-contract=off 
	 emcc b3main.o b3ogl.o b3emjs.o -shared -o mod.so -sMAIN_MODULE=2 -sWASM_WORKERS
	 emcc mod.so sdl.so -o g3020.js \
	 -fuse-ld=mold -sWASM_WORKERS -sUSE_SDL=2 -sFORCE_FILESYSTEM=1 -sALLOW_MEMORY_GROWTH=0 -sINITIAL_MEMORY=2048mb \
