...
COMMON_FLAGS += -pipe -sUSE_PTHREADS=1 -pthread -sWASM_WORKERS=1 -sSHARED_MEMORY=1 -sDISABLE_EXCEPTION_CATCHING=1 -fPIC -fpie -finline-functions -funroll-loops -fno-rtti \
...
LINK_FLAGS += $(LDFLAGS) -sWASMFS=0 -sASSERTIONS=0 -sPROXY_TO_PTHREAD=0 -fno-except -sPTHREAD_POOL_SIZE=64 --use-preload-plugins --closure 0 --closureFriendly \
	 -march=haswell -sWASM=1 -sTOTAL_STACK=16MB -sENVIRONMENT='web,webview,node,shell,worker' \
