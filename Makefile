...
SIMD_FLAGS += -DSIMD=1 -mrelaxed-simd -msse -msse2 -msse3 -mssse3 -msse4 -msse4.1 -msse4.2 -mavx -msimd128 
...
GL_FLAGS += -sFULL_ES2=0 -sFULL_ES3=1 -sUSE_GLFW=0 -Wl,-lGL,-lEGL,-lGLESv2 -sUSE_REGAL=0 \
	 -sUSE_WEBGL2=1 -sMIN_WEBGL_VERSION=2 -sMAX_WEBGL_VERSION=2 -sGL_TRACK_ERRORS=1 -sGL_MAX_TEMP_BUFFER_SIZE=262144

LINK_FLAGS += $(LDFLAGS) -sMEMFS_ASYNC=1 -sALLOW_DYNAMIC_LINKING=1 -sBINARYEN_ASYNC_COMPILATION=1 -sMALLOC=emmalloc -sOFFSCREENCANVAS_SUPPORT=1 -sTEXTDECODER=0 -sALLOW_TABLE_GROWTH=1 -sEMULATE_FUNCTION_POINTER_CASTS=0 -sSUPPORT_BIG_ENDIAN=1 \
...
b3_shader_webgpu:
	 ...
	 em++ $(STDS) -c src/shader/main.cpp $(COMMON_FLAGS) $(SIMD_FLAGS) $(BOOST_FLAGS) -DDOUBLE
	 em++ $(STDS) -DDOUBLE $(COMMON_FLAGS) $(LINK_FLAGS) $(GL_FLAGS) \
