...
LINK_FLAGS += $(LDFLAGS) -sALLOW_TABLE_GROWTH=1 -sEMULATE_FUNCTION_POINTER_CASTS=0 -sSUPPORT_BIG_ENDIAN=1 \
	 -sLZ4=1 -sTRUSTED_TYPES=1 -sALLOW_UNIMPLEMENTED_SYSCALLS=0 -sIGNORE_MISSING_MAIN=0 -sABORT_ON_WASM_EXCEPTIONS=0 -sDEMANGLE_SUPPORT=0 -sASSERTIONS=2 \
...
b3_vanilla_webgpu:
	 em++ lib/lib_webgpu_cpp20.cpp $(STDS) -static -matomics -mbulk-memory -pthread
	 em++ lib/lib_webgpu.cpp $(STDS) -static -matomics -mbulk-memory -pthread
	 ...
	 -std=c++17 $(BOOST_FLAGS) $(LINK_SIMD_FLAGS) -matomics -mbulk-memory -pthread -Wl,-lpthread -sFORCE_FILESYSTEM=1 \
