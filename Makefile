...
COMMON_FLAGS += -xc++ g++ -fopenmp-simd -sSUPPORT_LONGJMP=wasm -pthread -pipe -mextended-const -mbulk-memory -matomics \
...
LINK_FLAGS += --target=wasm32 -sMALLOC=emmalloc -DQUAD $(LDFLAGS) -sDEFAULT_TO_CXX=0 -sLEGALIZE_JS_FFI=0 -sOFFSCREENCANVAS_SUPPORT=0 \
