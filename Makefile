...
GL_FLAGS += -sFULL_ES3=1 -sFULL_ES2=0 -sUSE_SDL=0 -sLEGACY_GL_EMULATION=0 -sUSE_GLFW=0 -sUSE_REGAL=0 \
...
LINK_FLAGS += --target=wasm32 -sMALLOC=emmalloc $(LDFLAGS) -sDEFAULT_TO_CXX=1 -sLEGALIZE_JS_FFI=0 -sOFFSCREENCANVAS_SUPPORT=0 \
	 ...
	 --use-preload-plugins --closure 0 --closureFriendly -lpthreads \
...
b3_shader_webgpu:
	 ...
	 emcc $(STDS) -o $(BIN_NAME) -O2 $(COMMON_FLAGS) \
