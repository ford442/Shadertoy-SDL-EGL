b3hd:
	 em++ src/b3main.cpp -c \
         -fcoroutines-ts -std=gnu++2a -stdlib=libstdc++12 -mcpu=bleeding-edge -fwasm-exceptions -fexperimental-library \
	 -mbulk-memory -msign-ext -m32 -matomics -ffast-math -ffp-contract=fast -freciprocal-math
	 em++ src/b3ogl.cpp -c \
	 -fcoroutines-ts -std=gnu++2a -stdlib=libstdc++12 -matomics \
	 ...
	 em++ src/b3sdl.cpp -c \
	 -fcoroutines-ts -sUSE_SDL=2 -std=gnu++2a -stdlib=libstdc++12 -matomics -fslp-vectorize \
