...
for(var i=0;i<65;i++){
var j=i+1;
eval("var point"+j+"="+i+"*la;var $"+j+"=new Float32Array($H,point"+j+",la);$"+j+".set($$1,0,la);");
}
...
M();
if(getLoop==true){
if(mmvv.currentTime<(stp+loopPart)){
frameTimes[loopFrm]=vv.currentTime;
loopFrm+=1;
}else{
mmvv.pause();
getLoop=false;
...
mmvv.currentTime=frameTimes[loopFrm];
loopFrm-=1;
}else{
f=false;
};};
