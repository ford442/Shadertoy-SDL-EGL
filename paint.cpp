...
let glslAlphe=`float Alphe(float a,float b,float c,float d,float e,float f,float g){return ((0.85+(3.0*((1.0-b)-(((((1.0-f)-(a)+b)*1.5)/2.0)+((f-0.5)*((1.0-f)*0.25))-((0.5-f)*(f*0.25))-((g-e)*((1.0-g)*0.1))))))/4.0);}`;
...
var grr=(p[0]-0.7)+(p[1]-0.7)+(p[2]-0.7)-(p[0]-0.8)-(p[1]-0.8)-(p[2]-0.8);
grr=Math.min((grr*4.0),0.0);
var rr=p[0]-grr;
var gg=p[1]-grr;
var bb=p[2]-grr;
